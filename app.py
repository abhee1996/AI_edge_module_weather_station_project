...
#     # input_data = np.array([[temperature, humidity, pressure]])
#     # input_scaled = scaler.transform(input_data)
#     input_data = pd.DataFrame([[temperature, humidity, pressure]], columns=['temperature', 'humidity', 'pressure'])
#     input_scaled = scaler.transform(input_data)
#     prediction = model.predict(input_scaled)[0][0]  # Adjust for model type
#     #prediction = model.predict(input_scaled)[0]  # Adjust for model type

#     return prediction, {'temp': temperature, 'hum': humidity, 'press': pressure}

# app = Flask(__name__)

# @app.route('/')
# def index():
#     pred, data = predict_condition()
#     return render_template('index.html', data=data, pred=pred)

# @app.route('/plot')
# def plot():
#     _, data = predict_condition()  # Get fresh data
#     fig, ax = plt.subplots()
#     ax.bar(['Temp', 'Hum', 'Press'], [data['temp'], data['hum'], data['press']])
#     ax.set_ylabel('Values')
#     ax.set_title('Current Weather Data')
...
# def api_data():
#     pred, data = predict_condition()
#     return jsonify({'data': data, 'pred': pred})
